...
- **環境変数**: Linux/Mac での権限問題を避けるには、`.env` ファイルに `USER_ID=1000`（自分のUID）を記述してください。
  - ※ 自分の UID は、ターミナルで `id -u` と実行することで確認できます（通常は `1000` です）。
  - ※ Windows の場合は、デフォルト（root）のままで問題ありません。
